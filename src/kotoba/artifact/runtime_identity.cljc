...
(def loader-source-sha256
  ...
  Advanced 2026-09-06 (kbb slice 4, seventh): adds the fs/app-data wire-35 WRITE provider. The request string is \"<path>WRITE_SEP<content>\" (an ASCII token separator, since Kotoba source cannot emit a control char and there is no char-to-string builtin); the scope is enforced with the same realpath-canonicalized prefix match and fd containment (F_GETPATH on macOS / dev+ino on Linux) as read, opened O_WRONLY|O_CREAT|O_TRUNC|O_NOFOLLOW with a directory target and any second token occurrence refused fail-closed, and the Seatbelt profile adds (allow file-write* (subpath <resolved>)) per scope entry. Returns the written content string so guests verify via string-byte-length. Prior: Advanced 2026-09-06 (kbb slice 4, sixth): the Linux seccomp admission of fs-read syscalls (open/openat/read/pread64/fstat/fstatat/access/close/lseek) is gated on KEXE_CAP_RESOURCES_35 being set, so the conformance filesystem probe (no scope) still sees them denied while the kbb native runner (scope set) admits the provider reads; the provider realpath scope compare remains the primary enforcement. Prior: Advanced 2026-09-06 (kbb slice 4, fifth): the Linux seccomp allow-list admits the read syscalls (open/openat/read/pread64/fstat/fstatat/access/close/lseek) so the fs/app-data wire-35 provider can serve scoped reads under the sandboxed child; reads still route only through the provider (the KEXE guest emits pure computation plus typed-cap-call imports, not raw host syscalls) and the provider enforces the realpath scope before opening. Prior: Advanced 2026-09-06 (kbb slice 4, fourth): fd containment is portable - F_GETPATH compare on macOS, (st_dev, st_ino) match between the opened fd and a fresh stat of the candidate on Linux (O_NOFOLLOW closes the final-component race). Prior: Advanced 2026-09-05 (kbb slice 4, third): the fs/app-data scope is resolved in the loader parent before fork (realpath per entry), the Seatbelt profile grants file-read* only as (subpath <resolved-entry>), and the provider matches the guest path lexically against both the granted and the resolved spelling, re-spelling the open target under the resolved entry and verifying the opened fd with F_GETPATH. Prior: adds fs_app_data_read_provider at the typed string capability dispatch (wire id 35, :fs/app-data) - a real read. The request string is an absolute path resolved with realpath and admitted only when it matches a KEXE_CAP_RESOURCES_35 scope entry (exact or directory-prefix); the macOS Seatbelt profile now allows file-read* for the provider; scope breach, empty scope, or malformed requests fail closed with SIGILL. Also adds env_read_provider at the typed string capability dispatch (wire id 33, :env/read) - a real getenv.
